...
sub	debug
{
	print(@_)	if $debugflag;
}

sub	debugf
{
	printf(@_)	if $debugflag;
}
...
sub	add
{
	...
	if	(!$p)
	{ ## Substring is absent; we fell off the tree.
	...
	{ ## We're still on the tree, but the last letter may mismatch.
		my ($pOff, $pLen) = ($$p{off},$$p{len});
		my $pLetter = substr($target,$pOff+$sLeft-1,1);
		if	($sLetter eq $pLetter)
