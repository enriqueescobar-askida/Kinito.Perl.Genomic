...
#########################################
package SuffixTree;
##  This package implements suffix trees, an efficient data structure
##  for repeatedly searching for substrings of a fixed target string.
#########################################

use strict;

#########################################
## The following facilitate debugging.
## The subroutines of this package invoke debug and debugf with various
## trace information.  If $debugflag is true, it prints; otherwise, it doesn't.
#########################################
my $debugflag;
...
#########################################
sub new
##  creates a new hash holding a suffix tree for the given target string.
##  RETURNS: blessed reference to the new hash.
#########################################
{
	my ($this,    ## usually, the literal "SuffixTree"
	$target)  ## the string to be made searchable by the suffix tree
	= @_;
	my $root = {off=>0,len=>0};
	$$root{sfx} = $root;
	$target = lc $target;
...
#########################################
sub add {
##  Auxiliary subroutine to create the nodes of the suffix tree for
##  the target string.  Called by new.  Internal -- not a method.
#########################################
...
#########################################
sub isSubstring
##  Determines whether a query string is a substring of the target string
##  encoded in the suffix tree.
##  RETURNS: undef if query not substring
##           reference to node at which query found if a substring.
#########################################
{
	my ($this,        ## reference to this SuffixTree structure
	$query) = @_; ## string being sought as substring
	my $s = $this->{target};
	$query = lc $query;
	my $p = $this->{root};
...
##  Determines the starting points in the target string of the substring
##  represented by the given node $p.
##  RETURNS: list of the starting points.
#########################################
{
	my ($this,     ##  reference to this SuffixTree structure
	$p) = @_;  ##  reference to node within SuffixTree structure
	my @results;
	my @pending = ($p);
	while (@pending) {
	$p = shift @pending;
...
#########################################
sub dump
## dumps a suffix tree structure in human-readable form.
## RETURNS: nothing
#########################################
{
	my ($this) = @_;
...
#########################################
sub dumpNode {
## dumps a single node of a suffix tree on one line, appropriately indented.
## Internal -- not a method.
## RETURNS: nothing
#########################################
	my ($p,            ## node to dump.
	$s,            ## the target string.
	$indent) = @_; ## string of blanks for beginning of line.
